		...
		if (draw_bg) {
			pixman_image_fill_boxes(PIXMAN_OP_OVER, background,
						cur_bg_color, 1, &(pixman_box32_t){
							.x1 = x, .x2 = nx,
							//.y1 = 0, .y2 = buf_height //default
							.y1 = buf_height - 2, .y2 = buf_height //custom - line bar below text
						});
		}
...
//custom - hide ugly boxes above tags (start)
...
// 		}
// 	} 
//custom - hide ugly boxes above tags (end)
	...
	pixman_image_fill_boxes(PIXMAN_OP_SRC, background,
				...
				&(pixman_box32_t){
					.x1 = x, .x2 = nx,
					//.y1 = 0, .y2 = bar->height //default
					.y1 = bar->height - 2, .y2 = bar->height //custom - line bar below text
				});
	x = nx;
	
	x = draw_text(custom_title ? bar->title.text : bar->window_title,
		      x, y, foreground, foreground_mask, background,
		      //(bar->sel && active_color_title) ? &active_fg_color : &inactive_fg_color, //default
		      //(bar->sel && active_color_title) ? &active_bg_color : &inactive_bg_color, //default
		      (bar->sel && active_color_title) ? &inactive_fg_color : &inactive_fg_color, //custom - title color fix
		      (bar->sel && active_color_title) ? &inactive_bg_color : &inactive_bg_color, //custom - title color fix
